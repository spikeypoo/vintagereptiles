...
export default function ShopPageClient({ pages, initialCards }: ShopPageClientProps) {
  const initialMaxPrice = useMemo(() => {
    if (!initialCards.length) {
      return 0;
    }
    return Math.max(...initialCards.map((card) => parseFloat(card.price) || 0));
  }, [initialCards]);

  const [cards, setCards] = useState<ShopPageProduct[]>(initialCards);
  const [filteredCards, setFilteredCards] = useState<ShopPageProduct[]>(initialCards);
  ...
  const maxPrice = useMemo(() => {
    ...
    return Math.max(...cards.map((card) => parseFloat(card.price) || 0));
  }, [cards]);

  const [priceRange, setPriceRange] = useState<[number, number]>(() => [0, initialMaxPrice || 1000]);

  useEffect(() => {
    setCards(initialCards);
    setFilteredCards(initialCards);
    setVisibleCards(Math.min(20, initialCards.length));
    setPriceRange([0, initialMaxPrice || 1000]);
    setSearchTerm('');
    setSortOption('default');
    setShowInStock(false);
    setShowOnSale(false);
  }, [initialCards, initialMaxPrice]);
    ...
    <div className="px-4 py-8 md:px-8">
      {pages === 'prints' && (
        <div className="text-center text-white text-xl font-bold">
          To see all 3D Print colours,{' '}
          <span className="text-[#cb18db] cursor-pointer underline underline-offset-2">
            <Link href="/printcolours">click here</Link>
          </span>
        </div>
      )}
...
const ProductCard = ({
  ...
  const transformedImage = image
    ? image.replace('vintage-reptiles-storage.s3.us-east-2.amazonaws.com/', 'd3ke37ygqgdiqe.cloudfront.net/')
      ...
      <div className="bg-[#242122] rounded-lg overflow-hidden shadow-lg transform transition-transform duration-300 md:hover:scale-105 hover:shadow-xl">
        ...
        <div className="p-4">
          ...
          <div className="mt-2 flex flex-col items-center">
            {hasOptions ? (
              <>
                <span className="text-white font-bold text-lg">From ${parseFloat(price || '0').toFixed(2)}</span>
                <span className="text-gray-400 text-sm">{standardOptions.length} options available</span>
              </>
            ) : issale === 'true' ? (
              <div className="flex items-center">
                <span className="text-red-500 font-bold text-lg">${parseFloat(price || '0').toFixed(2)}</span>
                {oldprice && (
                  <span className="text-gray-400 line-through text-sm ml-2">
                    ${parseFloat(oldprice || '0').toFixed(2)}
                  </span>
                )}
              </div>
            ) : (
              price !== '' && <span className="text-white font-bold text-lg">${parseFloat(price || '0').toFixed(2)}</span>
            )}
          </div>
        </div>
      </div>
