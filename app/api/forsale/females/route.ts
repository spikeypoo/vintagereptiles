...
export async function GET() {
  ...
  const cursor = await prisma.females.findMany({
    select: {
      id: true,
      name: true,
      price: true,
...
export async function POST(request: Request) {
  ...
  const result = await client.db(mongoDbName).collection("Females").insertOne(doc);
  ...
  if (doc.price && doc.price !== "") {
    ...
    await client.db(mongoDbName).collection("Females").updateOne(
...
export async function PUT(request: Request) {
  ...
  // 4) Update DB
  const client = await connect;
  await client.db(mongoDbName).collection("Females").findOneAndUpdate(
    { _id: new ObjectId(id) },
    { $set: updateDoc }
  );
  ...
  const isExist = await client.db(mongoDbName).collection("Females").findOne({ _id: new ObjectId(id) });
  if (!isExist) {
...
export async function DELETE(request: Request) {
  ...
  await client.db(mongoDbName).collection("Females").findOneAndDelete({
    _id: new ObjectId(id),
  });
