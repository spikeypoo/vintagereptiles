...
export async function GET() {
  ...
  const cursor = await prisma.females.findMany({select: {id: true, name: true, price: true, image1: true , issale: true, oldprice: true, description: true, stock: true}, orderBy: {id: 'desc'}})
...
export async function POST(request: Request) {
  ...
  const result = await client.db("Products").collection("Females").insertOne(doc);
  ...
  if (doc.price && doc.price !== "") {
    ...
    await client.db("Products").collection("Females").updateOne(
...
export async function PUT(request: Request){
  ...
  if (await url4 != "")
    ...
    client.db("Products").collection("Females").findOneAndUpdate({"_id": new ObjectId(id)}, {$set: { "name": files.get("name"), "price": files.get("price"), "description": files.get("description"), "issale" : files.get("issale"), "oldprice" : files.get("oldprice"), "stock" : files.get("stock")}})
