...
export async function GET() {
  ...
  const cursor = await prisma.availability.findMany({
    select: {
      id: true,
      name: true,
      price: true,
...
export async function POST(request: Request) {
  ...
  const result = await client.db(mongoDbName).collection("Availability").insertOne(doc);
  ...
  if (doc.price && doc.price !== "") {
    ...
    await client.db(mongoDbName).collection("Availability").updateOne(
...
export async function PUT(request: Request) {
  ...
  // 4) Update DB
  const client = await connect;
  await client.db(mongoDbName).collection("Availability").findOneAndUpdate(
    { _id: new ObjectId(id) },
    { $set: updateDoc }
  );
  ...
  const isExist = await client.db(mongoDbName).collection("Availability").findOne({ _id: new ObjectId(id) });
  if (!isExist) {
...
export async function DELETE(request: Request) {
  ...
  await client.db(mongoDbName).collection("Availability").findOneAndDelete({
    _id: new ObjectId(id),
  });
