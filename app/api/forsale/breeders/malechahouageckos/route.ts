...
export async function POST(request: Request) {
  ...
  await client.db(mongoDbName).collection("Malechahouageckos").insertOne(doc);
...
export async function PUT(request: Request) {
  ...
  // 4) Update DB
  const client = await connect;
  await client.db(mongoDbName).collection("Malechahouageckos").findOneAndUpdate(
    { _id: new ObjectId(id) },
    { $set: updateDoc }
  );
  ...
  const isExist = await client.db(mongoDbName).collection("Malechahouageckos").findOne({ _id: new ObjectId(id) });
  if (!isExist) {
...
export async function DELETE(request: Request) {
  ...
  await client.db(mongoDbName).collection("Malechahouageckos").findOneAndDelete({
    _id: new ObjectId(id),
  });
