...
export async function GET() {
  ...
  const cursor = await prisma.malecrestedgeckos.findMany({
    select: {
      id: true,
      name: true,
      price: true,
...
export async function POST(request: Request) {
  ...
  await client.db(mongoDbName).collection("Malecrestedgeckos").insertOne(doc);
...
export async function PUT(request: Request) {
  ...
  // 4) Update DB
  const client = await connect;
  await client.db(mongoDbName).collection("Malecrestedgeckos").findOneAndUpdate(
    { _id: new ObjectId(id) },
    { $set: updateDoc }
  );
  ...
  const isExist = await client.db(mongoDbName).collection("Malecrestedgeckos").findOne({ _id: new ObjectId(id) });
  if (!isExist) {
...
export async function DELETE(request: Request) {
  ...
  await client.db(mongoDbName).collection("Malecrestedgeckos").findOneAndDelete({
    _id: new ObjectId(id),
  });
