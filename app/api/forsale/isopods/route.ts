...
export async function GET() {
  ...
  const cursor = await prisma.isopods.findMany({
    select: {
      id: true,
      name: true,
      price: true,
...
export async function POST(request: Request) {
  ...
  const result = await client.db(mongoDbName).collection("Isopods").insertOne(doc);
  ...
  if (doc.price && doc.price !== "") {
    ...
    await client.db(mongoDbName).collection("Isopods").updateOne(
...
export async function PUT(request: Request) {
  ...
  // 4) Update DB
  const client = await connect;
  await client.db(mongoDbName).collection("Isopods").findOneAndUpdate(
    { _id: new ObjectId(id) },
    { $set: updateDoc }
  );
  ...
  const isExist = await client.db(mongoDbName).collection("Isopods").findOne({ _id: new ObjectId(id) });
  if (!isExist) {
...
export async function DELETE(request: Request) {
  ...
  await client.db(mongoDbName).collection("Isopods").findOneAndDelete({
    _id: new ObjectId(id),
  });
