...
export async function POST(request: Request) {
  ...
  // 4) Connect + insert
  const client = await connect;
  await client.db("Products").collection("Isopods").insertOne(doc);
...
export async function PUT(request: Request) {
  ...
  // 4) Update DB
  const client = await connect;
  await client.db("Products").collection("Isopods").findOneAndUpdate(
    { _id: new ObjectId(id) },
    { $set: updateDoc }
  );
  ...
  const isExist = await client.db("Products").collection("Isopods").findOne({ _id: new ObjectId(id) });
  if (!isExist) {
...
export async function DELETE(request: Request) {
  ...
  await client.db("Products").collection("Isopods").findOneAndDelete({
    _id: new ObjectId(id),
  });
